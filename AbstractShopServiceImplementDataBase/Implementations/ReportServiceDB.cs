...
{
    class ReportServiceDB : IReportService
    {
        ...
        public List<StocksLoadViewModel> GetStocksLoad()
        {
            ...
            .ToList()
           .GroupJoin(context.StockComponents
 .Include(r => r.Component)
.ToList(),
 stock => stock,
stockComponent => stockComponent.Stock, // Stock
(stock, stockCompList) =>
 new StocksLoadViewModel
 {
    ...
    int>(r.ComponentId.ToString(), r.Count)) // Component.Componentname
 })
 .ToList();
        }
        ...
        public List<ClientOrdersModel> GetClientOrders(ReportBindingModel model)
        {
           ...
           rec.DateCreate <= model.DateTo)
            ...
            {
                ClientName = rec.Client.CustomerFIO,
           ...
           rec.DateCreate),
                ProductName = rec.Service.ServiceName,
                Count = rec.Count,
                Sum = rec.Sum,
        ...
        public void SaveClientOrders(ReportBindingModel model)
        {
           ...
           iTextSharp.text.Font.BOLD);
            ...
            for (int i = 0; i < list.Count; i++)
            {
                ...
                table.AddCell(cell); cell = new PdfPCell(new Phrase(list[i].Sum.ToString(), fontForCells));
                cell.HorizontalAlignment = Element.ALIGN_RIGHT;
                table.AddCell(cell);
